...
function StoolUrineTestExecution() {
  ...
  // Machine animation - only GIF, no static images
  const [machineImage] = useState('/StoolTest.gif')
  ...
  // Both use the same 8 parameters from Stool Test reference table
  const generateTestResults = () => {
    return {
      fecalOccultBlood: {
        value: Math.random() < 0.9 ? 'Negative' : 'Positive',
        normalRange: 'Negative',
        unit: '',
        status: Math.random() < 0.9 ? 'normal' : 'abnormal'
      },
      leukocytes: {
        value: Math.random() < 0.85 ? 'Negative / None seen' : `${(1 + Math.random() * 10).toFixed(0)} WBCs/HPF`,
        normalRange: 'Negative / None seen',
        unit: '',
        status: Math.random() < 0.85 ? 'normal' : 'abnormal'
      },
      fecalFat: {
        value: (Math.random() * 10).toFixed(1),
        normalRange: '< 7 g/24h (varies by diet)',
        unit: 'g/24h',
        status: Math.random() * 10 < 7 ? 'normal' : 'abnormal'
      },
      ovaAndParasites: {
        value: Math.random() < 0.95 ? 'Negative / No ova or parasites seen' : 'Positive - Giardia/Amoebas detected',
        normalRange: 'Negative / No ova or parasites seen',
        unit: '',
        status: Math.random() < 0.95 ? 'normal' : 'abnormal'
      },
      pH: {
        value: (7.0 + Math.random() * 0.5).toFixed(1), // 7.0 - 7.5
        normalRange: '7.0-7.5 (Neutral to slightly alkaline)',
        unit: '',
        status: 'normal'
      },
      reducingSubstances: {
      ...
      fecalCalprotectin: {
        value: (Math.random() * 60).toFixed(1),
        normalRange: '< 50 mcg/g',
        unit: 'mcg/g',
        status: Math.random() * 60 < 50 ? 'normal' : 'abnormal'
      },
      colorConsistency: {
        value: Math.random() < 0.8 ? 'Brown / Formed' : ['Black/Clay', 'Yellow/Watery'][Math.floor(Math.random() * 2)],
        normalRange: 'Brown / Formed',
        unit: '',
        status: Math.random() < 0.8 ? 'normal' : 'abnormal'
      }
    }
  }
  ...
  const startTest = () => {
    ...
    // setMachineImage('/StoolTest.gif') - already set in state
    ...
    setTimeout(() => {
      ...
      // setMachineImage('/anh4.jpg') - REMOVED
    }, totalMs)
  }
  ...
  const handleSaveResults = async () => {
    ...
    try {
      ...
      const resultsData = {
        ...
        // Extract Stool/Urine test fields from results (for individual database fields)
        fobt_value: testResults.fecalOccultBlood?.value || null,
        wbcs_value: testResults.leukocytes?.value || null,
        fecal_fat: testResults.fecalFat?.value ? parseFloat(testResults.fecalFat.value) : null,
        O_and_P: testResults.ovaAndParasites?.value || null,
        ph_value: testResults.pH?.value ? parseFloat(testResults.pH.value) : null,
        rs_value: testResults.reducingSubstances?.value || null,
        fc_value: testResults.fecalCalprotectin?.value ? parseFloat(testResults.fecalCalprotectin.value) : null,
        color: testResults.colorConsistency?.value || null,
            ...
            <div className="p-3 sm:p-4 md:p-6 overflow-y-auto flex-1">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4 mb-4 md:mb-6">
                {Object.entries(testResults).map(([key, result]: [string, any]) => (
                  ...
                  >
                    <div className="text-xs sm:text-sm text-gray-500 mb-1 capitalize">
